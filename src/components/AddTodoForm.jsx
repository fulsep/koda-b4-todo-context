  ...
  return (
    ...
    <form ref={formRef} onSubmit={handleData} className='flex flex-col gap-2'>
      <div className='flex flex-col gap-1'>
        ...
        <input autoFocus className='h-12 border rounded px-3' type="text" name='title' id='title' defaultValue={isEditMode ? editData.title : ''}
        />
      </div>
      <div className='flex flex-col gap-1'>
        <label htmlFor="body">Body</label>
        <textarea className='border rounded p-3 h-24' type="text" name='body' id='body' defaultValue={isEditMode ? editData.body : ''}
        />
      </div>
      <div className='flex flex-col gap-1'>
        <label htmlFor="time">Time</label>
        <select className='border h-12 rounded px-3' name="time" id="time" defaultValue={isEditMode ? editData.time : '00:00'}
        >
          {[...Array(24)].map((_, hour) => {
            const strHour = hour.toString()
            const len = strHour.length
            const timeValue = len > 1 ? strHour : '0' + strHour
            return (<option key={hour} value={`${timeValue}:00`}>{timeValue}:00</option>)
          })}
        </select>
      </div>
